...
- **Target product**: https://agentdeck.app/ — the commercial product we are building toward feature parity with
...
- `~/Applications/Agent Deck.app` is a local bundle wrapper around the repo-built binary, not a separate product.
- When launching `Agent Deck.app`, refresh the bundle first with `zsh scripts/launch-dev-app.sh` instead of only `open -na` (avoids stale binaries).
...
- `/Applications/Agent Deck.app` and `https://agentdeck.app/` are closed-source reference baselines only — behavior benchmarks, not the development runtime.
