...
struct CodexSessionTrackingTests {
    ...
    func codexSessionStoreRoundTripsTrackedSessions() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-tracking-\(UUID().uuidString)", isDirectory: true)
        ...
        let records = [
            CodexTrackedSessionRecord(
                ...
                jumpTarget: JumpTarget(
                    ...
                    paneTitle: "codex ~/Personal/agent-deck"
                ),
    ...
    @Test
    func codexPromptNormalizerExtractsRequestFromUploadedFileWrapper() {
        let prompt = """
        # Files mentioned by the user:

        ## 8a7d6826-5790-4d22-89d0-553430828ab9.jpeg: /var/folders/tmp/image.jpeg

        ## My request for Codex:
        修正 灵动岛像素级偏移次一个字节长度需要超出
        """

        #expect(
            CodexPromptNormalizer.userVisiblePrompt(from: prompt)
                == "修正 灵动岛像素级偏移次一个字节长度需要超出"
        )
    }
    ...
    func codexSessionStoreLoadsLegacyRecordsWithoutAttachmentState() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-legacy-tracking-\(UUID().uuidString)", isDirectory: true)
        let fileURL = rootURL.appendingPathComponent("session-terminals.json")
        let store = CodexSessionStore(fileURL: fileURL)
    ...
    @Test
    func codexRolloutReducerUsesVisibleRequestForUploadedImagePrompt() {
        let snapshot = CodexRolloutReducer.snapshot(for: [
            rolloutLine(
                timestamp: "2026-04-17T09:58:19.000Z",
                type: "event_msg",
                payload: [
                    "type": "user_message",
                    "message": """
        ...
        #expect(snapshot.initialUserPrompt == "修正 灵动岛像素级偏移次一个字节长度需要超出")
        #expect(snapshot.lastUserPrompt == "修正 灵动岛像素级偏移次一个字节长度需要超出")
        #expect(snapshot.summary == "Prompt: 修正 灵动岛像素级偏移次一个字节长度需要超出")
    }

    @Test
    func codexRolloutWatcherTracksAppendedLines() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-rollout-\(UUID().uuidString)", isDirectory: true)
    ...
    func codexRolloutWatcherBootstrapsPromptMetadataFromHeadWhenTailMissesIt() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-rollout-head-bootstrap-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
    ...
    func codexRolloutWatcherBootstrapsFromBoundedTailWindow() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-rollout-tail-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
    ...
    func codexRolloutDiscoveryFindsRecentSessionsFromLocalRollouts() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-discovery-\(UUID().uuidString)", isDirectory: true)
        ...
        #expect(records.first?.title == "Codex · agent-deck")
    ...
    @Test
    func codexRolloutDiscoveryUsesThreadNameFromCodexSessionIndex() throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-discovery-title-\(UUID().uuidString)", isDirectory: true)
        let sessionDirectoryURL = rootURL.appendingPathComponent("2026/04/17", isDirectory: true)
        let rolloutURL = sessionDirectoryURL.appendingPathComponent("rollout-title.jsonl")
        let indexURL = rootURL.appendingPathComponent("session_index.jsonl")
        let now = Date(timeIntervalSince1970: 1_776_421_121)
        ...
        try [
            sessionMetaLine(
                sessionID: "019d9ae0-89b9-7be2-beaa-e95518a3699e",
                timestamp: "2026-04-17T09:58:19.000Z",
                cwd: "/Users/xilong/Desktop/agentdeck"
            ),
        ]
            .joined(separator: "\n")
            .appending("\n")
        ...
        try """
        {"id":"019d9ae0-89b9-7be2-beaa-e95518a3699e","thread_name":"修正灵动岛像素级偏移次一个字节长度需要超出","updated_at":"2026-04-17T09:58:41.993757Z"}
        """
            .write(to: indexURL, atomically: true, encoding: .utf8)

        let discovery = CodexRolloutDiscovery(
            rootURL: rootURL,
            sessionIndex: CodexSessionIndex(fileURL: indexURL),
            maxAge: 86_400,
            maxFiles: 10
        )
        ...
        #expect(records.first?.title == "Codex · 修正灵动岛像素级偏移次一个字节长度需要超出")
        #expect(records.first?.codexMetadata?.threadName == "修正灵动岛像素级偏移次一个字节长度需要超出")
    }
}
