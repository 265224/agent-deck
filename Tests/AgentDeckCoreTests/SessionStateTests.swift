...
struct SessionStateTests {
    ...
    @Test
    func hookManagedSessionStaysVisibleDuringMissingProcessGrace() {
        let startedAt = Date(timeIntervalSince1970: 1_000)
        var state = SessionState()
        ...
        state.apply(
            .sessionStarted(
                SessionStarted(
                    ...
                    tool: .codex,
                    origin: .live,
                    summary: "Codex received a new prompt.",
                    timestamp: startedAt
                )
            )
        )

        _ = state.markProcessLiveness(aliveSessionIDs: [], at: startedAt.addingTimeInterval(4))
        _ = state.markProcessLiveness(aliveSessionIDs: [], at: startedAt.addingTimeInterval(6))
        _ = state.removeInvisibleSessions()

        #expect(state.liveSessionCount == 1)
        #expect(state.session(id: "codex-desktop-session")?.isSessionEnded == false)
        #expect(state.session(id: "codex-desktop-session")?.processNotSeenCount == 2)
    }

    @Test
    func hookManagedSessionEndsAfterMissingProcessGraceExpires() {
        let startedAt = Date(timeIntervalSince1970: 1_000)
        var state = SessionState()
        ...
        let staleTime = startedAt.addingTimeInterval(SessionState.hookManagedMissingProcessGraceInterval + 1)
        _ = state.markProcessLiveness(aliveSessionIDs: [], at: staleTime)
        _ = state.markProcessLiveness(aliveSessionIDs: [], at: staleTime.addingTimeInterval(2))
        _ = state.removeInvisibleSessions()

        #expect(state.liveSessionCount == 0)
        #expect(state.session(id: "stale-codex-session") == nil)
    }

    @Test
    func codexInternalTitleGenerationSessionIsHidden() {
        let prompt = """
        ...
        let session = AgentSession(
            id: "codex-title-helper",
            title: "Codex · agentdeck",
            tool: .codex,
            origin: .live,
            attachmentState: .attached,
            phase: .running,
            summary: "{\"title\":\"撰写1000字小说\"}",
        ...
        #expect(session.isInternalCodexHelperSession)
        #expect(session.isVisibleInIsland == false)
        #expect(session.isTrackedLiveSession == false)
        #expect(state.liveSessionCount == 0)

        _ = state.removeInvisibleSessions()
        #expect(state.session(id: "codex-title-helper") == nil)
    }

    @Test
    func codexTitleGenerationPromptIsDetectedFromRolloutSnapshot() {
        let line = """
        {"timestamp":"2026-04-17T09:00:00.000Z","type":"event_msg","payload":{"type":"user_message","message":"You are a helpful assistant. You will be presented with a user prompt, and your job is to provide a short title for a task that will be created from that prompt.\\n\\nUser prompt:\\n测试标题过滤"}}
        """

        let snapshot = CodexRolloutReducer.snapshot(for: [line])

        #expect(CodexInternalPromptClassifier.containsTitleGenerationPrompt(inRawText: line))
        #expect(CodexInternalPromptClassifier.isTitleGenerationPrompt(snapshot.initialUserPrompt))
    }
    ...
    @Test
    func deleteSessionRemovesSessionFromState() {
        let stateDate = Date(timeIntervalSince1970: 2_400)
        let deleteSession = AgentSession(
        ...
        var keepSession = AgentSession(
            id: "session-to-keep",
            title: "Keep me",
            tool: .claudeCode,
            phase: .running,
            summary: "Working",
            updatedAt: stateDate.addingTimeInterval(5)
        )
        keepSession.isProcessAlive = true
        var state = SessionState(sessions: [deleteSession, keepSession])

        let didDeleteSession = state.deleteSession(id: "session-to-delete")
        #expect(didDeleteSession)

        #expect(state.session(id: "session-to-delete") == nil)
        #expect(state.session(id: "session-to-keep") != nil)
        #expect(state.liveSessionCount == 1)
        let didDeleteMissingSession = state.deleteSession(id: "missing-session")
        #expect(!didDeleteMissingSession)
    }
    ...
    func actionableStateResolvedClearsWaitingForApproval() {
        ...
        #expect(state.session(id: "claude-approval")?.summary == "Approval was handled outside Agent Deck.")
    }
    ...
    func codexPreToolUseWaitsForApprovalAndReturnsDenyDirective() async throws {
        ...
        #expect(response == .codexHookDirective(.deny(reason: "Permission denied in Agent Deck.")))
    }
    ...
    @Test
    func codexStopWithoutTaskCompleteKeepsTurnRunning() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-codex-stop-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
        try """
        {"timestamp":"2026-04-17T09:00:00.000Z","type":"event_msg","payload":{"type":"user_message","message":"Keep working until the task is actually complete."}}
        {"timestamp":"2026-04-17T09:00:01.000Z","type":"event_msg","payload":{"type":"agent_message","message":"I am checking the code now."}}
        ...
        #expect(startedEvent.isSessionStarted)
        #expect(activityEvent.activityUpdate?.sessionID == "codex-stop-running")
        #expect(activityEvent.activityUpdate?.phase == .running)
        #expect(activityEvent.sessionCompleted == nil)
    }

    @Test
    func codexStopWithTaskCompleteEmitsCompletion() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("agent-deck-codex-complete-\(UUID().uuidString)", isDirectory: true)
        let rolloutURL = rootURL.appendingPathComponent("rollout.jsonl")
        try FileManager.default.createDirectory(at: rootURL, withIntermediateDirectories: true)
        try """
        {"timestamp":"2026-04-17T09:00:00.000Z","type":"event_msg","payload":{"type":"user_message","message":"Finish and summarize."}}
        {"timestamp":"2026-04-17T09:00:02.000Z","type":"event_msg","payload":{"type":"task_complete","last_agent_message":"Finished and verified."}}
        ...
        #expect(startedEvent.isSessionStarted)
        #expect(completionEvent.sessionCompleted?.sessionID == "codex-stop-complete")
        #expect(completionEvent.sessionCompleted?.summary == "Finished and verified.")
        #expect(completionEvent.sessionCompleted?.isInterrupt != true)
    }
    ...
    func codexHookInstallerMergesManagedGroupsWithoutDroppingUnrelatedHooks() throws {
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            ...
            hookCommand: "'/tmp/AgentDeckHooks'"
        )
        ...
        let managedStopHook = stopGroups?
            .compactMap { $0["hooks"] as? [[String: Any]] }
            .flatMap { $0 }
            .first(where: { $0["command"] as? String == "'/tmp/AgentDeckHooks'" })

        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/AgentDeckHooks'"))
    ...
    @Test
    func codexHookInstallerInstallReplacesLegacyAgentDeckCommands() throws {
        let existing = """
        {
          "hooks": {
            "PreToolUse": [
              {
                ...
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/Users/test/.agent-deck/bin/agent-deck-bridge' --source codex"
                  },
                  {
                    "type": "command",
            ...
            "Stop": [
              {
                "hooks": [
                  ...
                  {
                    "type": "command",
                    "command": "'/tmp/old-debug/AgentDeckHooks'"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true"
        ...
        let mutation = try CodexHookInstaller.installHooksJSON(
            existingData: existing,
            hookCommand: "'/tmp/new-release/AgentDeckHooks'"
        )
        ...
        #expect(hooks?["PostToolUse"] == nil)
        #expect(stopCommands.contains("/usr/bin/true"))
        #expect(stopCommands.contains("'/tmp/new-release/AgentDeckHooks'"))
        #expect(!stopCommands.contains("'/Users/test/.agent-deck/bin/agent-deck-bridge' --source codex"))
        #expect(!stopCommands.contains("'/tmp/old-debug/AgentDeckHooks'"))
    }
    ...
    func codexHookInstallerUninstallRemovesOnlyManagedHooks() throws {
            ...
            "Stop": [
              {
                "hooks": [
                  {
                    "type": "command",
                    "command": "'/tmp/AgentDeckHooks'",
                    "statusMessage": "Managed by Agent Deck"
                  },
                  {
                    "type": "command",
                    "command": "/usr/bin/true",
        ...
        let mutation = try CodexHookInstaller.uninstallHooksJSON(
            existingData: existing,
            managedCommand: "'/tmp/AgentDeckHooks'"
        )
    ...
    func codexHookPayloadCarriesSessionLocatorIntoJumpTarget() {
        let payload = CodexHookPayload(
            ...
            terminalTitle: "codex ~/P/agent-deck",
            transcriptPath: nil
        )
        ...
        #expect(jumpTarget.paneTitle == "codex ~/P/agent-deck")
    }
