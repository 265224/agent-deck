...
final class TerminalJumpServiceTests: XCTestCase {
    ...
    func testGhosttyJumpScriptFallsBackToWorkingDirectoryAndTitle() {
        ...
        XCTAssertTrue(script.contains("(working directory of aTerminal as text) is \"/Users/wangruobing/Personal/agent-deck\""))
        XCTAssertTrue(script.contains("(name of aTerminal as text) contains \"codex ~/p/agent-deck\""))
        XCTAssertTrue(script.contains("if \"\" is \"\" then"))
    }

    func testGhosttyJumpIntegrationMatchesFocusedTerminalForLiveSurfaces() throws {
        guard ProcessInfo.processInfo.environment["AGENT_DECK_RUN_GHOSTTY_JUMP_INTEGRATION"] == "1" else {
            throw XCTSkip("Set AGENT_DECK_RUN_GHOSTTY_JUMP_INTEGRATION=1 to run live Ghostty jump verification.")
        }
    ...
    func testCursorJumpActivatesRunningAppWithoutWorkspaceReuse() throws {
        ...
        let result = try service.jump(
            to: JumpTarget(
                ...
                workingDirectory: "/Users/test/agent-deck"
            )
        )
    ...
    func testTraeCNJumpFallsBackToWorkspaceViaTraeCLI() throws {
        ...
        XCTAssertEqual(processInvocations.values.first?.1, ["-r", "/Users/test/agent-deck"])
    }
}
