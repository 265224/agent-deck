...
struct AppModelSessionListTests {
    ...
    func islandListDeduplicatesSessionsSharingTheSameLiveGhosttyTerminal() {
        ...
        #expect(model.surfacedSessions.map(\.id) == ["running-live"])
        #expect(model.recentSessions.map(\.id).contains("old-turn-same-split"))
        #expect(model.recentSessions.map(\.id).contains("other-live"))
    ...
    @Test
    func completedLiveSessionsDoNotStayInThePrimaryIslandList() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()

        var completedSession = AgentSession(
        ...
        completedSession.isProcessAlive = true

        model.state = SessionState(sessions: [completedSession])
        model.notchStatus = .opened
        model.notchOpenReason = .notification
        model.islandSurface = .sessionList(actionableSessionID: completedSession.id)

        #expect(model.surfacedSessions.isEmpty)
        #expect(model.islandListSessions.isEmpty)
        #expect(model.liveSessionCount == 0)
        #expect(model.recentSessions.map(\.id) == ["completed-session"])
        #expect(model.activeIslandCardSession?.id == "completed-session")
    }

    @Test
    func completionNotificationBindsCompletedSessionWhileOtherSessionsRun() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()
        ...
        let completingSession = AgentSession(
        ...
        var stillRunningSession = AgentSession(
            id: "still-running-session",
            title: "CatPaw · agentdeck",
            tool: .catPaw,
            origin: .live,
            attachmentState: .attached,
            phase: .running,
        ...
        stillRunningSession.isProcessAlive = true
        model.state = SessionState(sessions: [completingSession, stillRunningSession])
        ...
        #expect(model.islandSurface == .sessionList(actionableSessionID: "completing-session"))
        #expect(model.completionNotificationSession?.id == "completing-session")
        #expect(model.liveRunningCount == 1)
        #expect(model.surfacedSessions.map(\.id) == ["still-running-session"])

        model.notchClose()
        #expect(model.completionNotificationSession == nil)
    }

    @Test
    func islandListDoesNotDeduplicateSessionsWithUnknownTerminalIdentity() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()
        let sharedUnknownJumpTarget = JumpTarget(
            terminalApp: "Unknown",
            workspaceName: "agentdeck",
            paneTitle: "Codex 019d9ad1",
            workingDirectory: "/tmp/agentdeck"
        )

        var firstLiveSession = AgentSession(
            id: "codex-session-a",
            ...
            attachmentState: .attached,
            phase: .running,
            summary: "Working on session A.",
            updatedAt: now,
            jumpTarget: sharedUnknownJumpTarget
        )
        firstLiveSession.isProcessAlive = true

        var secondLiveSession = AgentSession(
            id: "codex-session-b",
            title: "Codex · agentdeck",
            tool: .codex,
            origin: .live,
            attachmentState: .attached,
            phase: .running,
            summary: "Working on session B.",
            updatedAt: now.addingTimeInterval(-1),
            jumpTarget: sharedUnknownJumpTarget
        )
        secondLiveSession.isProcessAlive = true

        model.state = SessionState(sessions: [firstLiveSession, secondLiveSession])

        #expect(model.surfacedSessions.map(\.id) == ["codex-session-a", "codex-session-b"])
        #expect(model.liveSessionCount == 2)
    }

    @Test
    func islandListSkipsEmptyCodexBootstrapSessions() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel()

        var blankBootstrapSession = AgentSession(
            id: "blank-bootstrap",
            title: "Codex · agentdeck",
            tool: .codex,
            origin: .live,
            attachmentState: .attached,
            phase: .running,
            summary: "Started Codex session in agentdeck.",
            updatedAt: now,
            jumpTarget: JumpTarget(
                terminalApp: "Unknown",
                workspaceName: "agentdeck",
                paneTitle: "Codex 27975b2c",
                workingDirectory: "/tmp/agentdeck"
            )
        )
        blankBootstrapSession.isProcessAlive = true
        ...
        var activeSession = AgentSession(
            ...
            jumpTarget: JumpTarget(
                terminalApp: "Unknown",
                workspaceName: "agentdeck",
                paneTitle: "Codex a0dde5cc",
                workingDirectory: "/tmp/agentdeck"
            ),
            codexMetadata: CodexSessionMetadata(
                lastUserPrompt: "我只有两个项目在运行，它里面多了一个空白的"
            )
        )
        activeSession.isProcessAlive = true

        model.state = SessionState(sessions: [blankBootstrapSession, activeSession])

        #expect(model.surfacedSessions.map(\.id) == ["active-session"])
        #expect(!model.recentSessions.map(\.id).contains("blank-bootstrap"))
        #expect(model.liveSessionCount == 1)
    }
    ...
    @Test
    func completionNotificationBypassesFrontmostSuppression() {
        let now = Date(timeIntervalSince1970: 2_000)
        let model = AppModel(
        ...
        #expect(model.completionNotificationSession?.id == "frontmost-session")
    }
    ...
    func mergedWithSyntheticClaudeSessionsSkipsSyntheticWhenAttachedClaudeAlreadyRepresentsGroup() {
        ...
        let existing = AgentSession(
            ...
            jumpTarget: JumpTarget(
                terminalApp: "Ghostty",
                workspaceName: "agent-deck",
                paneTitle: "agent-deck · hi · e45d5e87",
                workingDirectory: "/tmp/agent-deck",
                terminalSessionID: "ghostty-claude"
            )
        )
    ...
    func mergedWithSyntheticClaudeSessionsSkipsSyntheticWhenStaleClaudeSessionMatchesActiveProcess() {
        ...
        let existing = AgentSession(
            id: "e45d5e87-66d0-4f67-8399-6ebc02f3d453",
            title: "Claude · agent-deck-readme",
            tool: .claudeCode,
            origin: .live,
            attachmentState: .stale,
            phase: .completed,
            ...
            jumpTarget: JumpTarget(
                terminalApp: "Ghostty",
                workspaceName: "agent-deck-readme",
                paneTitle: "Claude e45d5e87",
                workingDirectory: "/tmp/agent-deck-readme"
            ),
            claudeMetadata: ClaudeSessionMetadata(
