...
struct OverlayPanelControllerTests {
    ...
    @Test
    func islandClosedHeightCoversMenuBarWhenItIsTallerThanSafeArea() {
        // Simulates MacBook Air M2: physical notch ≈ 34 pt, menu bar reserved ≈ 37 pt.
        // Use the taller reserved area so the island bottom aligns with the menu-bar edge.
    ...
    @Test
    func notchWidthUsesExactAuxiliaryGap() {
        let width = NSScreen.computeNotchWidth(
            screenFrame: NSRect(x: 0, y: 0, width: 1_470, height: 956),
    ...
    @Test
    func notchWidthFallsBackToScreenMinusAuxiliaryWidths() {
        let width = NSScreen.computeNotchWidth(
            screenFrame: NSRect(x: 0, y: 0, width: 1_470, height: 956),
            auxiliaryTopLeftArea: NSRect(x: 0, y: 924, width: 646, height: 32),
    ...
    @Test
    func notchCenterUsesExactAuxiliaryGapMidpoint() {
        let centerX = NSScreen.computeNotchCenterX(
            screenFrame: NSRect(x: 0, y: 0, width: 1_470, height: 956),
            auxiliaryTopLeftArea: NSRect(x: 0, y: 924, width: 646, height: 32),
            auxiliaryTopRightArea: NSRect(x: 825, y: 924, width: 645, height: 32)
        )

        #expect(centerX == 735.5)
    }

    @Test
    func notchCenterFallsBackToScreenMidX() {
        let centerX = NSScreen.computeNotchCenterX(
            screenFrame: NSRect(x: 10, y: 0, width: 1_470, height: 956),
