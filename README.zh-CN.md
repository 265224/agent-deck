...
<p align="center">
  <strong>面向 Codex 和 coding agents 的开源 macOS 控制面板。</strong>
  <br>
  本地优先、原生实现，为真实 agent 维护工作流而构建。
  <br><br>
...
<p align="center">
  <img src="docs/images/demo.gif" alt="Agent Deck 运行效果" width="720">
</p>
...
Agent Deck 驻留在 Mac 的**刘海区域**（或顶部栏），为 Codex 和其他 AI coding agents 提供实时控制面板：会话状态、权限审批、用量窗口、一键跳回正确的终端。全程不打断你的工作流。

它是一个开源、本地优先的替代方案，适合希望把 agent 工作流、遥测数据和自动化 hooks 留在自己机器上的开发者。
...
- **多 Agent** — 一个界面管理 Codex、Claude Code、Cursor、CatPaw、Gemini CLI、OpenCode 等
- **多终端** — 一键跳回到准确的终端/IDE 会话

## Codex 工作流支持

Agent Deck 把 Codex 当作一等集成，而不是普通进程监控：

- **受管 Codex hooks** — 在 `~/.codex/config.toml` 中安装和卸载 `SessionStart`、`UserPromptSubmit`、`Stop` hooks
- **本地会话发现** — 读取 `~/.codex/sessions/**/rollout-*.jsonl` 下的 Codex rollout 文件，展示会话和用量状态
- **权限和状态面板** — 展示实时会话状态、通知，并快速跳回负责该任务的终端或 IDE
- **Fail-open bridge** — hook payload 通过本地 Unix socket 转发；Agent Deck 关闭时，Codex 仍会正常运行

## 支持的 Agents 和终端

**10 个 Agents**：Claude Code、Codex、Cursor、CatPaw、Gemini CLI、OpenCode、Qoder、Qwen Code、Factory、CodeBuddy

**16+ 终端和 IDE**：Terminal.app、Ghostty、iTerm2、WezTerm、Zellij、tmux、cmux、Kaku、VS Code、Cursor、CatPaw、Windsurf、Trae、JetBrains 全家桶（IDEA、WebStorm、PyCharm、GoLand、CLion、RubyMine、PhpStorm、Rider、RustRover）
...
| **CatPaw** | 已支持 | Hook 集成，通过 CatPaw 全局 `hooks.json` 配置，会话追踪，工作区跳转，不做用量追踪 |
...
| **CatPaw** | 工作区 | 通过 `catpaw` CLI 激活工作区 |
...
| 发布工作流 | DMG 打包、Sparkle appcast、签名和公证文档 |
...
首次启动时，Agent Deck 会自动发现活跃的 agent 会话并启动 live bridge。Hook 安装在 app 内的**控制中心**管理。

> **系统要求**：macOS 14+、Swift 6.2、Xcode

### Release 构建

签名公证后的 DMG 会在可用时通过 [GitHub Releases](https://github.com/265224/agent-deck/releases) 发布。发布流程见 [docs/releasing.md](docs/releasing.md)。
...
Hooks **fail open**——如果 Agent Deck 没在运行，你的 agents 不受任何影响。
...
| **AgentDeckCore** | 共享库——模型、bridge 传输（Unix socket IPC）、hooks、会话持久化 |
| **AgentDeckHooks** | 轻量 CLI，由 agent hooks 调用，通过 Unix socket 转发 payload |
| **AgentDeckSetup** | 安装器 CLI，管理 `~/.codex/config.toml` 和 hook entries |
...
```
我在使用 Agent Deck (https://github.com/265224/agent-deck) 时遇到了问题。
...
1. 收集我的环境信息：
   ...
   - 检查 Agent Deck 是否在运行：`ps aux | grep -i "agentdeck\|AgentDeckApp" | grep -v grep`
   - 获取 app 版本：`defaults read ~/Applications/Agent\ Deck.app/Contents/Info.plist CFBundleShortVersionString 2>/dev/null || echo "unknown"`
   - 检查我当前使用的终端
...
`Agent Deck` 会在刘海区域或顶部栏放置一个轻量控制界面，让你可以在不中断当前 flow 的前提下观察 live coding agents、跟踪会话进度，并快速跳回正确的 terminal 上下文。
...
- **CatPaw** — 基于 hook 的集成，通过 CatPaw 全局 `hooks.json` 配置。接收 shell、MCP、文件、prompt、stop、response 和 thought 事件。支持会话追踪和工作区跳转；不接入用量 / 余量追踪。使用 `--source catpaw` 调用 hooks binary。
...
该脚本会创建 `output/package/Agent Deck.app` 和 `output/package/Agent Deck.zip`。传入 `AGENT_DECK_SIGN_IDENTITY` 可以签名。详见 [docs/packaging.md](docs/packaging.md)。
...
控制中心展示来自 `~/.codex` 的实时 Codex hook 安装状态，并可直接安装或卸载受管 hook entries。安装过程会把 helper 复制到 `~/Library/Application Support/AgentDeck/bin/AgentDeckHooks`，repo 重命名不会破坏已有 hooks。
...
Claude usage 设置可在 app 控制中心启用，保持 opt-in。bridge 会把受管 `statusLine.command` 写入 `~/.agent-deck/bin/agent-deck-statusline`，把 `rate_limits` 缓存到 `/tmp/agent-deck-rl.json`，不会自动覆盖已有的自定义 status line。
