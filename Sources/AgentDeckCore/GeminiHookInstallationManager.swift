...
public final class GeminiHookInstallationManager: @unchecked Sendable {
    ...
    public func install(hooksBinaryURL: URL) throws -> GeminiHookInstallationStatus {
        ...
        let legacyManifestURL = geminiDirectory.appendingPathComponent(GeminiHookInstallerManifest.legacyFileName)
    ...
    public func uninstall() throws -> GeminiHookInstallationStatus {
        let settingsURL = geminiDirectory.appendingPathComponent("settings.json")
        let manifestURL = resolvedManifestURL()
        let primaryManifestURL = geminiDirectory.appendingPathComponent(GeminiHookInstallerManifest.fileName)
        let legacyManifestURL = geminiDirectory.appendingPathComponent(GeminiHookInstallerManifest.legacyFileName)
        let manifest = try loadManifest(at: manifestURL)
        let existingSettings = try? Data(contentsOf: settingsURL)
    ...
    private func resolvedManifestURL() -> URL {
        let primaryURL = geminiDirectory.appendingPathComponent(GeminiHookInstallerManifest.fileName)
        if fileManager.fileExists(atPath: primaryURL.path) {
            return primaryURL
        }

        let legacyURL = geminiDirectory.appendingPathComponent(GeminiHookInstallerManifest.legacyFileName)
