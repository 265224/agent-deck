...
public struct SessionState: Equatable, Sendable {
    static let hookManagedMissingProcessGraceInterval: TimeInterval = 30 * 60
    ...
    public mutating func apply(_ event: AgentEvent) {
        ...
        case let .sessionMetadataUpdated(payload):
            ...
            if let threadName = payload.codexMetadata.threadName?.trimmingCharacters(in: .whitespacesAndNewlines),
               !threadName.isEmpty {
                session.title = "Codex · \(threadName)"
            }
            session.updatedAt = payload.timestamp
            upsert(session)
    ...
    public mutating func resolvePermission(
        ...
        if resolution.isApproved {
            ...
            switch session.tool {
            case .claudeCode, .geminiCLI, .qoder, .qwenCode, .factory, .codebuddy:
                session.summary = "Permission denied in Agent Deck."
            case .openCode:
                session.summary = "Permission denied in Agent Deck."
            default:
        ...
        for (id, var session) in sessionsByID {
            ...
            if session.isHookManaged {
                ...
                if aliveSessionIDs.contains(id) {
                    ...
                    session.processNotSeenCount = min(session.processNotSeenCount + 1, 2)
                    let isWithinHookGrace = now.timeIntervalSince(session.updatedAt) < Self.hookManagedMissingProcessGraceInterval
                    if !isWithinHookGrace, session.processNotSeenCount >= 2 {
                        session.isSessionEnded = true
                        session.phase = .completed
                        changed.insert(id)
                    }
    ...
    /// Permanently remove a session from the current island state.
    @discardableResult
    public mutating func deleteSession(id: String) -> Bool {
        sessionsByID.removeValue(forKey: id) != nil
    }
