...
public enum CodexInternalPromptClassifier {
    private static let titleGenerationPromptIntro =
    ...
    private static let titleGenerationPromptTaskFragment =
        "job is to provide a short tit"

    public static func isTitleGenerationPrompt(_ prompt: String?) -> Bool {
        guard let prompt = prompt.map(normalizedPromptText)?.trimmingCharacters(in: .whitespacesAndNewlines),
              !prompt.isEmpty else {
            return false
        }

        return (prompt.hasPrefix(titleGenerationPromptPrefix) && prompt.contains("User prompt:"))
            || (prompt.hasPrefix(titleGenerationPromptIntro) && prompt.contains(titleGenerationPromptTaskFragment))
    }

    public static func containsTitleGenerationPrompt(inRawText text: String) -> Bool {
        let normalized = normalizedPromptText(text)

        return normalized.contains(titleGenerationPromptPrefix)
            && normalized.contains("User prompt:")
    }
...
public struct CodexSessionMetadata: Equatable, Codable, Sendable {
    ...
    public var isInternalTitleGenerationRequest: Bool {
        CodexInternalPromptClassifier.isTitleGenerationPrompt(initialUserPrompt)
            || CodexInternalPromptClassifier.isTitleGenerationPrompt(lastUserPrompt)
    }
}
...
public extension CodexTrackedSessionRecord {
    ...
    var shouldRestoreToLiveState: Bool {
        origin != .demo
            ...
            && codexMetadata?.isInternalTitleGenerationRequest != true
    }
}
...
public final class CodexRolloutDiscovery: @unchecked Sendable {
    ...
    private struct SessionMeta {
        ...
        func sessionTitle(threadName: String?) -> String {
            guard let threadName,
                  !threadName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                return "Codex · \(workspaceName)"
            }

            return "Codex · \(threadName)"
        }
    ...
    private let fileManager: FileManager
    private let sessionIndex: CodexSessionIndex
    ...
    public init(
        ...
        fileManager: FileManager = .default,
        sessionIndex: CodexSessionIndex = CodexSessionIndex(),
    ...
    private func discoverRecord(fileURL: URL, modifiedAt: Date) -> CodexTrackedSessionRecord? {
        ...
        let threadName = sessionIndex.threadName(for: sessionMeta.sessionID)
        ...
        return CodexTrackedSessionRecord(
            ...
            title: sessionMeta.sessionTitle(threadName: threadName),
            origin: .live,
...
public enum CodexRolloutReducer {
    ...
    private static func applyEventMessage(
        ...
        case "user_message":
            guard let message = clipped(CodexPromptNormalizer.userVisiblePrompt(from: payload["message"] as? String)), !message.isEmpty else {
                break
            }
    ...
    private static func applyUserMessage(
        ...
        let visibleMessage = CodexPromptNormalizer.userVisiblePrompt(from: message) ?? message
        snapshot.initialUserPrompt = snapshot.initialUserPrompt ?? visibleMessage
        snapshot.lastUserPrompt = visibleMessage
        snapshot.currentTool = nil
        snapshot.currentCommandPreview = nil
        snapshot.phase = .running
        snapshot.isCompleted = false
        snapshot.isInterrupted = false
        snapshot.summary = "Prompt: \(visibleMessage)"
    ...
    private static func responseMessageText(
        ...
        let visible = textType == "input_text"
            ? (CodexPromptNormalizer.userVisiblePrompt(from: joined) ?? joined)
            : joined
...
public final class CodexRolloutWatcher: @unchecked Sendable {
    ...
    private let sessionIndex: CodexSessionIndex
    private let queue = DispatchQueue(label: "app.agentdeck.codex.rollout-watcher")
    ...
    private func refresh(observation: inout Observation) -> [AgentEvent] {
        var events = threadNameEventsIfNeeded(observation: &observation)
    ...
    private func threadNameEventsIfNeeded(observation: inout Observation) -> [AgentEvent] {
        let latestThreadName = sessionIndex.threadName(for: observation.target.sessionID)
        guard latestThreadName != observation.threadName else {
            return []
        }

        observation.threadName = latestThreadName
        guard let latestThreadName else {
            return []
        }
            ...
            .sessionMetadataUpdated(
                SessionMetadataUpdated(
                    ...
                    codexMetadata: CodexSessionMetadata(
                        transcriptPath: observation.target.transcriptPath,
                        threadName: latestThreadName,
                        initialUserPrompt: observation.snapshot.initialUserPrompt,
                        lastUserPrompt: observation.snapshot.lastUserPrompt,
                        lastAssistantMessage: observation.snapshot.lastAssistantMessage,
                        currentTool: observation.snapshot.currentTool,
                        currentCommandPreview: observation.snapshot.currentCommandPreview
                    ),
                    timestamp: .now
                )
            ),
    ...
    private func makeObservation(for target: CodexRolloutWatchTarget) -> Observation {
        ...
        guard FileManager.default.fileExists(atPath: fileURL.path),
            ...
            return Observation(
                target: target,
                threadName: sessionIndex.threadName(for: target.sessionID)
            )
        }
