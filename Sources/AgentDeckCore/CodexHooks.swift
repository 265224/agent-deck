...
public extension CodexHookPayload {
    var isInternalTitleGenerationRequest: Bool {
        CodexInternalPromptClassifier.isTitleGenerationPrompt(prompt)
    }
    ...
    var sessionTitle: String {
        if let threadName = CodexSessionIndex().threadName(for: sessionID) {
    ...
    var defaultJumpTarget: JumpTarget {
        JumpTarget(
            ...
            paneTitle: terminalTitle ?? "Codex \(sessionDisplayID)",
    ...
    var defaultCodexMetadata: CodexSessionMetadata {
        let visiblePrompt = CodexPromptNormalizer.userVisiblePrompt(from: prompt)
        return CodexSessionMetadata(
            transcriptPath: transcriptPath,
            threadName: CodexSessionIndex().threadName(for: sessionID),
            initialUserPrompt: visiblePrompt ?? promptPreview,
            lastUserPrompt: visiblePrompt ?? promptPreview,
    ...
    var promptPreview: String? {
        clipped(CodexPromptNormalizer.userVisiblePrompt(from: prompt) ?? prompt)
    }
