...
public struct HookHealthReport: Equatable, Sendable {
    ...
    public enum Issue: Equatable, Sendable, CustomStringConvertible {
        ...
        /// Other hooks detected alongside Agent Deck hooks (informational).
...
public enum HookHealthCheck {
    ...
    public static func checkClaude(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: settingsPath),
           hasAgentDeckHooks(in: settingsURL, fileManager: fileManager) {
    ...
    public static func checkCodex(
        ...
        // 3. Check manifest
        if fileManager.fileExists(atPath: hooksPath),
           hasAgentDeckHooks(in: hooksURL, fileManager: fileManager) {
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Only check Agent Deck / Agent Deck commands
                    let normalized = command.lowercased()
                    guard normalized.contains("agentdeckhooks") || normalized.contains("agentdeckhooks")
                        || normalized.contains("agent-deck") || normalized.contains("agent-deck") else {
                        continue
                    }
    ...
    /// Finds third-party (non-Agent Deck) hook command names for display.
        ...
        for (_, eventValue) in hooks {
            ...
            for group in groups {
                ...
                for hook in hookEntries {
                    ...
                    // Skip Agent Deck / Agent Deck hooks
                    if normalized.contains("agentdeckhooks") || normalized.contains("agentdeckhooks")
                        || normalized.contains("agent-deck") || normalized.contains("agent-deck") {
                        continue
                    }
    ...
    /// Checks whether any Agent Deck managed hooks exist in a config file.
    private static func hasAgentDeckHooks(in url: URL, fileManager: FileManager) -> Bool {
