...
public enum BridgeSocketLocation {
    ...
    private static var stableDirectoryURL: URL {
        ...
        return appSupport.appendingPathComponent("AgentDeck")
    }
    ...
    public static var legacyURL: URL {
        URL(fileURLWithPath: "/tmp/agent-deck-\(getuid()).sock")
    }

    public static var legacyStableURL: URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? URL(fileURLWithPath: NSHomeDirectory()).appendingPathComponent("Library/Application Support")
        return appSupport
    ...
    public static func currentURL(environment: [String: String] = ProcessInfo.processInfo.environment) -> URL {
        if let path = environment["AGENT_DECK_SOCKET_PATH"], !path.isEmpty {
    ...
    public static func uniqueTestURL() -> URL {
        URL(fileURLWithPath: "/tmp/agent-deck-test-\(UUID().uuidString).sock")
    }
}
...
public enum BridgeCommand: Equatable, Codable, Sendable {
    ...
    case processCatPawHook(CatPawHookPayload)
    ...
    public init(from decoder: any Decoder) throws {
        ...
        case .processCatPawHook:
            self = .processCatPawHook(try container.decode(CatPawHookPayload.self, forKey: .catPawHook))
        }
    }
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        case let .processCatPawHook(payload):
            try container.encode(CommandType.processCatPawHook, forKey: .type)
            try container.encode(payload, forKey: .catPawHook)
        }
    }
}
...
public enum BridgeResponse: Equatable, Codable, Sendable {
    ...
    case catPawHookDirective(CatPawHookDirective)
    ...
    public init(from decoder: any Decoder) throws {
        ...
        case .catPawHookDirective:
            self = .catPawHookDirective(try container.decode(CatPawHookDirective.self, forKey: .directive))
        }
    }
    ...
    public func encode(to encoder: any Encoder) throws {
        ...
        case let .cursorHookDirective(directive):
            ...
            try container.encode(directive, forKey: .directive)
        case let .catPawHookDirective(directive):
            try container.encode(ResponseType.catPawHookDirective, forKey: .type)
            try container.encode(directive, forKey: .directive)
        }
    }
}
