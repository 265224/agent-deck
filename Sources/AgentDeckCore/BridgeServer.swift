...
public final class BridgeServer: @unchecked Sendable {
    private static let catPawCompletionResponseSuppressionWindow: TimeInterval = 10
    ...
    private let queue = DispatchQueue(label: "app.agentdeck.bridge.server")
    ...
    private var recentlyCompletedCatPawSessions: [String: Date] = [:]
    ...
    public func start() throws {
        ...
        // Keep legacy sockets alive so already-running sessions and old
        // AgentDeck-installed hooks can still connect after the rename.
        for legacyURL in [BridgeSocketLocation.legacyStableURL, BridgeSocketLocation.legacyURL]
            where legacyURL != socketURL {
    ...
    private func stopLocked() {
        ...
        recentlyCompletedCatPawSessions.removeAll()
    ...
    private func handle(_ command: BridgeCommand, from clientID: UUID) {
        ...
        case let .resolvePermission(sessionID, resolution):
            ...
            if let interaction = pendingCursorInteractions.removeValue(forKey: sessionID) {
                ...
                case let .deny(message, _):
                    ...
                    summary = message ?? "Permission denied in Agent Deck."
                    phase = .completed
                }
            ...
            broadcast([.event(
                resolution.isApproved
                    ...
                    : .sessionCompleted(
                        SessionCompleted(
                            sessionID: sessionID,
                            summary: "Permission denied in Agent Deck.",
                            timestamp: .now
                        )
                    )
            )])
        ...
        case let .processCatPawHook(payload):
            handleCatPawHook(payload, from: clientID)
        }
    }

    private func handleCodexHook(_ payload: CodexHookPayload, from clientID: UUID) {
        guard !shouldIgnoreCodexHook(payload) else {
            send(.response(.acknowledged), to: clientID)
            return
        }
        ...
        case .stop:
            ...
            if let completionEvent = codexCompletionEventFromTranscript(for: payload) {
                emit(completionEvent)
            } else {
                let summary = payload.lastAssistantMessage
                    ?? payload.assistantMessagePreview
                    ?? "Codex is thinking."
                emit(
                    .activityUpdated(
                        SessionActivityUpdated(
    ...
    private func shouldIgnoreCodexHook(_ payload: CodexHookPayload) -> Bool {
        if ignoredCodexSessionIDs.contains(payload.sessionID) {
            return true
        }

        if payload.isInternalTitleGenerationRequest
            || codexTranscriptIsInternalTitleGenerationRequest(payload.transcriptPath) {
    ...
    private func codexTranscriptIsInternalTitleGenerationRequest(_ transcriptPath: String?) -> Bool {
        guard let transcriptPath,
              let raw = try? String(contentsOfFile: transcriptPath, encoding: .utf8) else {
            return false
        }

        if CodexInternalPromptClassifier.containsTitleGenerationPrompt(inRawText: raw) {
            return true
        }
        ...
        return CodexInternalPromptClassifier.isTitleGenerationPrompt(snapshot.initialUserPrompt)
            || CodexInternalPromptClassifier.isTitleGenerationPrompt(snapshot.lastUserPrompt)
    }
    ...
    private func handleCatPawHook(_ payload: CatPawHookPayload, from clientID: UUID) {
        let now = Date()
        pruneRecentlyCompletedCatPawSessions(now: now)

        switch payload.hookEventName {
        case .beforeSubmitPrompt:
            recentlyCompletedCatPawSessions.removeValue(forKey: payload.sessionID)
            ensureCatPawSessionExists(for: payload)
            synchronizeCatPawJumpTarget(for: payload)
            ...
            send(.response(.catPawHookDirective(.allow)), to: clientID)

        case .beforeShellExecution:
            ensureCatPawSessionExists(for: payload)
            synchronizeCatPawJumpTarget(for: payload)
            synchronizeCatPawMetadata(for: payload)
            let summary = payload.commandPreview.map { "Running: \($0)" } ?? "Running shell command"
            emit(
                .activityUpdated(
                    SessionActivityUpdated(
                        sessionID: payload.sessionID,
        ...
        case .afterShellExecution:
            ...
            synchronizeCatPawJumpTarget(for: payload)
            synchronizeCatPawMetadata(for: payload)
            let command = payload.commandPreview ?? "shell command"
            let summary = payload.outputPreview.map { "Shell finished: \(command) · \($0)" } ?? "Shell finished: \(command)"
            emit(
                .activityUpdated(
                    SessionActivityUpdated(
                        sessionID: payload.sessionID,
        ...
        case .afterMCPExecution:
            ...
            synchronizeCatPawJumpTarget(for: payload)
            synchronizeCatPawMetadata(for: payload)
            let tool = payload.toolName ?? "MCP tool"
            let summary = payload.resultPreview.map { "\(tool) finished: \($0)" } ?? "\(tool) finished."
            emit(
                .activityUpdated(
                    SessionActivityUpdated(
                        sessionID: payload.sessionID,
        ...
        case .afterFileEdit:
            ...
            synchronizeCatPawJumpTarget(for: payload)
            synchronizeCatPawMetadata(for: payload)
            let summary = payload.filePath.map { "Edited \($0)" } ?? "CatPaw edited a file"
            emit(
                .activityUpdated(
                    SessionActivityUpdated(
                        sessionID: payload.sessionID,
        ...
        case .afterAgentResponse:
            ...
            // CatPaw emits afterAgentResponse for intermediate assistant
            // messages before the turn stops. Treat stop as the authoritative
            // completion signal so active thinking sessions stay surfaced.
            if !wasRecentlyCompletedCatPawSession(payload.sessionID, now: now),
               localState.session(id: payload.sessionID)?.phase != .completed {
                emit(
                    .activityUpdated(
                        SessionActivityUpdated(
                            sessionID: payload.sessionID,
                            summary: payload.assistantTextPreview ?? "CatPaw responded.",
                            phase: .running,
                            timestamp: now
                        )
                    )
        ...
        case .stop:
            ...
            switch payload.status {
            case "error":
                stopSummary = "CatPaw encountered an error."
            case "aborted":
                stopSummary = "CatPaw task was aborted."
            default:
                stopSummary = payload.assistantTextPreview ?? "CatPaw completed the turn."
            }
            recentlyCompletedCatPawSessions[payload.sessionID] = now
            emit(
                .sessionCompleted(
                    SessionCompleted(
                        sessionID: payload.sessionID,
            ...
            send(.response(.catPawHookDirective(CatPawHookDirective(continue: nil, followupMessage: ""))), to: clientID)
        }
    }

    private func wasRecentlyCompletedCatPawSession(_ sessionID: String, now: Date) -> Bool {
        guard let completedAt = recentlyCompletedCatPawSessions[sessionID] else {
            return false
        }

        return now.timeIntervalSince(completedAt) <= Self.catPawCompletionResponseSuppressionWindow
    }

    private func pruneRecentlyCompletedCatPawSessions(now: Date) {
        recentlyCompletedCatPawSessions = recentlyCompletedCatPawSessions.filter { _, completedAt in
            now.timeIntervalSince(completedAt) <= Self.catPawCompletionResponseSuppressionWindow
        }
    }
    ...
    private func ensureCatPawSessionExists(for payload: CatPawHookPayload) {
        guard !hasSession(id: payload.sessionID) else {
            return
        }
        ...
        emit(
            .sessionStarted(
                SessionStarted(
                    ...
                    timestamp: .now,
                    jumpTarget: payload.defaultJumpTarget,
                    catPawMetadata: payload.defaultCatPawMetadata.isEmpty ? nil : payload.defaultCatPawMetadata
                )
            )
        )
    }

    private func synchronizeCatPawJumpTarget(for payload: CatPawHookPayload) {
    ...
    private func synchronizeCatPawMetadata(for payload: CatPawHookPayload) {
        let existing = localState.session(id: payload.sessionID)?.catPawMetadata
        let update = payload.defaultCatPawMetadata

        let merged = CatPawSessionMetadata(
            conversationId: update.conversationId ?? existing?.conversationId,
            suggestUuid: update.suggestUuid ?? existing?.suggestUuid,
            ...
            currentTool: mergedCatPawCurrentTool(
                existing: existing?.currentTool,
                update: update.currentTool,
                hookEventName: payload.hookEventName
            ),
            currentToolInputPreview: mergedCatPawCurrentToolInputPreview(
                existing: existing?.currentToolInputPreview,
                update: update.currentToolInputPreview,
                hookEventName: payload.hookEventName
            ),
            currentCommandPreview: mergedCatPawCurrentCommandPreview(
    ...
    private func mergedCatPawCurrentTool(
        existing: String?,
        update: String?,
        ...
        case .afterShellExecution, .afterMCPExecution, .afterFileEdit, .afterAgentResponse, .stop:
            return nil
        case .beforeShellExecution, .beforeMCPExecution, .beforeReadFile, .beforeSubmitPrompt, .afterAgentThought:
            return existing
        }
    }

    private func mergedCatPawCurrentToolInputPreview(
        existing: String?,
        update: String?,
        hookEventName: CatPawHookEventName
    ) -> String? {
    ...
    private func mergedCatPawCurrentCommandPreview(
        existing: String?,
        update: String?,
        hookEventName: CatPawHookEventName
    ) -> String? {
        ...
        switch hookEventName {
        case .afterShellExecution, .afterAgentResponse, .stop:
            return nil
        case .beforeShellExecution, .beforeMCPExecution, .afterMCPExecution, .beforeReadFile, .afterFileEdit, .beforeSubmitPrompt, .afterAgentThought:
            return existing
        }
    }
    ...
    private func clearStaleCursorInteractionIfNeeded(for sessionID: String) {
        ...
        emit(
            .actionableStateResolved(
                ActionableStateResolved(
                    sessionID: sessionID,
                    summary: "Approval was handled outside Agent Deck.",
                    timestamp: .now
                )
            )
        )
    ...
    private func resolvePendingOpenCodeInteraction(
        ...
        case let (.permission(_), .deny(message, _)):
            directive = .deny(reason: message ?? "Permission denied in Agent Deck.")
            summary = message ?? "Permission denied in Agent Deck."
            phase = .completed
    ...
    private func codexCompletionEventFromTranscript(for payload: CodexHookPayload) -> AgentEvent? {
        ...
        return .sessionCompleted(
            SessionCompleted(
                ...
                isInterrupt: snapshot.isInterrupted
            )
        )
    }
    ...
    private func resolvePendingApproval(sessionID: String, approved: Bool) {
        ...
        if approved {
            ...
            response = .codexHookDirective(.deny(reason: "Permission denied in Agent Deck."))
        }
    ...
    private func resolvePendingClaudeInteraction(
        ...
        case let (.permission(_), .deny(message, interrupt)):
            directive = .permissionRequest(
                .deny(message: message ?? "Permission denied in Agent Deck.", interrupt: interrupt)
            )
            summary = message ?? "Permission denied in Agent Deck."
            phase = .completed
