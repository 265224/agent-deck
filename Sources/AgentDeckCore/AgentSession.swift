...
public extension AgentSession {
    ...
    var isInternalCodexHelperSession: Bool {
        tool == .codex && codexMetadata?.isInternalTitleGenerationRequest == true
    }

    var isTrackedLiveSession: Bool {
        !isDemoSession
            && !isInternalCodexHelperSession
            && (tool == .codex || tool == .claudeCode || tool == .geminiCLI || tool == .openCode || tool == .qoder || tool == .qwenCode || tool == .factory || tool == .codebuddy || tool == .cursor || tool == .catPaw)
    }

    var isTrackedLiveCodexSession: Bool {
        tool == .codex && !isDemoSession && !isInternalCodexHelperSession
    }
    ...
    var isVisibleInIsland: Bool {
        ...
        if isInternalCodexHelperSession { return false }
    ...
    var currentToolName: String? {
        if let value = codexMetadata?.currentTool { return value }
        if let value = claudeMetadata?.currentTool { return value }
        if let value = openCodeMetadata?.currentTool { return value }
        if let value = cursorMetadata?.currentTool { return value }
        if let value = catPawMetadata?.currentTool { return value }
        return nil
    }
    ...
    var lastAssistantMessageText: String? {
        ...
        if let value = catPawMetadata?.lastAssistantMessage { return value }
        return nil
    }
    ...
    var trackingTranscriptPath: String? {
        if let value = codexMetadata?.transcriptPath { return value }
        if let value = claudeMetadata?.transcriptPath { return value }
        if let value = geminiMetadata?.transcriptPath { return value }
        return nil
    }
    ...
    var latestUserPromptText: String? {
        ...
        if let value = catPawMetadata?.lastUserPrompt { return value }
        return nil
    }
    ...
    var initialUserPromptText: String? {
        ...
        if let value = catPawMetadata?.initialUserPrompt { return value }
        return nil
    }

    var currentCommandPreviewText: String? {
        if let value = codexMetadata?.currentCommandPreview { return value }
        if let value = claudeMetadata?.currentToolInputPreview { return value }
        if let value = openCodeMetadata?.currentToolInputPreview { return value }
        if let value = cursorMetadata?.currentToolInputPreview { return value }
        if let value = catPawMetadata?.currentToolInputPreview { return value }
        return nil
    }
}
