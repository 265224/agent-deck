...
public final class CursorHookInstallationManager: @unchecked Sendable {
    ...
    public func install(hooksBinaryURL: URL) throws -> CursorHookInstallationStatus {
        ...
        let legacyManifestURL = cursorDirectory.appendingPathComponent(CursorHookInstallerManifest.legacyFileName)
    ...
    public func uninstall() throws -> CursorHookInstallationStatus {
        let hooksURL = cursorDirectory.appendingPathComponent("hooks.json")
        let manifestURL = resolvedManifestURL()
        let primaryManifestURL = cursorDirectory.appendingPathComponent(CursorHookInstallerManifest.fileName)
        let legacyManifestURL = cursorDirectory.appendingPathComponent(CursorHookInstallerManifest.legacyFileName)
        let manifest = try loadManifest(at: manifestURL)
        let existingHooks = try? Data(contentsOf: hooksURL)
    ...
    private func resolvedManifestURL() -> URL {
        let primaryURL = cursorDirectory.appendingPathComponent(CursorHookInstallerManifest.fileName)
        if fileManager.fileExists(atPath: primaryURL.path) {
            return primaryURL
        }

        let legacyURL = cursorDirectory.appendingPathComponent(CursorHookInstallerManifest.legacyFileName)
