...
struct AgentDeckHooksCLI {
    ...
    private enum HookSource: String {
        ...
        var isClaudeFormat: Bool {
            ...
            case .claude, .qoder, .qwen, .factory, .droid, .codebuddy:
                return true
            case .codex, .cursor, .gemini, .catpaw:
                return false
            }
        }
    }
    ...
    static func main() {
        do {
            ...
            case .catpaw:
                let payload = try decoder.decode(CatPawHookPayload.self, from: input)
                let response = try? client.send(.processCatPawHook(payload), timeout: 45)

                if let output = try CatPawHookOutputEncoder.standardOutput(
                    for: response,
    ...
    private static func logStderr(_ message: String) {
        guard let data = "[AgentDeckHooks] \(message)\n".data(using: .utf8) else { return }
