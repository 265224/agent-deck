...
final class ProcessMonitoringCoordinator {
    ...
    func liveAttachmentKey(for session: AgentSession) -> String? {
        ...
        if let warpPaneUUID = jumpTarget.warpPaneUUID?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !warpPaneUUID.isEmpty {
            return "\(terminalApp.lowercased()):warp:\(warpPaneUUID.lowercased())"
        }

        if let tmuxTarget = jumpTarget.tmuxTarget?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !tmuxTarget.isEmpty {
            let socket = jumpTarget.tmuxSocketPath?
                .trimmingCharacters(in: .whitespacesAndNewlines)
                .lowercased() ?? ""
            return "\(terminalApp.lowercased()):tmux:\(socket):\(tmuxTarget.lowercased())"
        }
        ...
        guard supportedTerminalApp(for: jumpTarget.terminalApp) != nil else {
            return nil
        }
