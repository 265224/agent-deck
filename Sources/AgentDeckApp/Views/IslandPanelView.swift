...
struct OverlayRootView: View {
    ...
    var body: some View {
        switch model.overlayPresentationMode {
        case .notch:
            IslandPanelView(model: model)
        case .desktopPet:
            DesktopPetPanelView(model: model)
        }
    }
}
...
private enum DesktopPetVisualState {
    case idleWave
    case idlePlay
    case busy
    case success
...
private struct DesktopPetPanelView: View {
    private static let petSize = DesktopPetMetrics.petSize
    private static let bubbleMaxWidth: CGFloat = 492
    private static let bubbleMaxHeight: CGFloat = 382
    private static let refreshInterval: TimeInterval = 1

    var model: AppModel

    @State private var isPetHovering = false
    ...
    private var spotlightSession: AgentSession? {
    ...
    private var hasBusyTask: Bool {
        model.liveSessionCount > 0
    }

    private var hasAttention: Bool {
        model.liveAttentionCount > 0
    }
    ...
    var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .bottom) {
                ...
                if isOpened {
                    VStack(spacing: 10) {
                        Spacer(minLength: 0)
                        taskBubble(availableSize: geometry.size)
        ...
        .animation(openAnimation, value: model.notchStatus)
    }

    private func taskBubble(availableSize: CGSize) -> some View {
        let width = min(Self.bubbleMaxWidth, max(260, availableSize.width - 28))
        let maxContentHeight = min(Self.bubbleMaxHeight, max(160, availableSize.height - Self.petSize.height - 52))
        ...
        return VStack(spacing: 0) {
            ...
            bubbleContent(maxHeight: maxContentHeight)
                .padding(12)
        }
        .frame(width: width)
        .background(
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 24, style: .continuous)
                .strokeBorder(Color.white.opacity(hasAttention ? 0.16 : 0.08), lineWidth: 1)
        )
    }
    ...
    private var bubbleHeader: some View {
        HStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                bubbleIconButton(systemName: "gearshape.fill", tint: .white.opacity(0.62)) {
                    model.showSettings()
                }

                bubbleIconButton(systemName: "xmark", tint: .white.opacity(0.62)) {
                    model.notchClose()
                }
            }
        }
    ...
    @ViewBuilder
    private func bubbleContent(maxHeight: CGFloat) -> some View {
        if model.shouldShowSessionBootstrapPlaceholder {
            desktopPetEmptyState(
                title: model.lang.t("island.checkingTerminals"),
                detail: model.lang.t("island.terminalOwnership"),
                showsProgress: true
            )
            .frame(height: min(maxHeight, 128))
        } else if isNotificationMode, model.activeIslandCardSession != nil {
            sessionList(maxHeight: maxHeight)
        } else if model.islandListSessions.isEmpty {
            desktopPetEmptyState(
                title: model.lang.t("desktopPet.noTasks"),
                detail: model.recentSessions.isEmpty
                    ? model.lang.t("island.startAgent")
                    : model.lang.t("island.recentSessions")
            )
            .frame(height: min(maxHeight, 118))
        } else {
            sessionList(maxHeight: maxHeight)
        }
    }

    private func sessionList(maxHeight: CGFloat) -> some View {
        TimelineView(.periodic(from: .now, by: Self.refreshInterval)) { context in
            AutoHeightScrollView(maxHeight: maxHeight) {
                VStack(spacing: 4) {
                    if isNotificationMode, let session = model.activeIslandCardSession {
                        ...
                        let expandedListCount = model.islandListSessions.count
                        let currentSessionIsInExpandedList = model.islandListSessions.contains { $0.id == session.id }
                        let minimumExpandableCount = currentSessionIsInExpandedList ? 2 : 1
                        if expandedListCount >= minimumExpandableCount {
                            Button {
                                model.expandNotificationToSessionList(clearExpansion: session.phase == .completed)
                            } label: {
                                Text(model.lang.t("island.showAll", expandedListCount))
    ...
    private func desktopPetEmptyState(
        title: String,
        detail: String,
    ...
    private var petAvatar: some View {
        if showsCompletionState(at: .now) {
            staticPetAvatar
        } else if hasBusyTask {
            animatedBusyPetAvatar
        } else {
            staticPetAvatar
        }
    }

    private var animatedBusyPetAvatar: some View {
        TimelineView(.animation(minimumInterval: 0.12, paused: false)) { context in
            let bob = CGFloat(sin(context.date.timeIntervalSinceReferenceDate * 7.0)) * 3
            petAvatarContent(visualState: .busy, date: context.date, bob: bob)
        }
    }

    private var staticPetAvatar: some View {
        TimelineView(.periodic(from: .now, by: 1)) { context in
            let visualState = visualState(at: context.date)
            petAvatarContent(visualState: visualState, date: context.date, bob: 0)
        }
    }

    private func petAvatarContent(visualState: DesktopPetVisualState, date: Date, bob: CGFloat) -> some View {
        let isBusy = visualState == .busy
        let tint = statusTint(for: visualState)

        return ZStack(alignment: .topTrailing) {
            petImage(for: visualState, tint: tint)
                .frame(width: Self.petSize.width, height: Self.petSize.height)
                .opacity(isPetHovering ? 0.96 : 1)
                .offset(y: bob)

            if isBusy {
                ClosedCountBadge(
                    liveCount: model.liveSessionCount,
                    tint: hasAttention ? .orange : tint
                )
            ...
            if hasAttention {
                AttentionIndicator(size: 18, color: .orange)
                    .padding(7)
            ...
            if isBusy {
                busyDots(at: date, tint: tint)
                    .offset(x: -24, y: Self.petSize.height - 25)
            }
        }
        .frame(width: Self.petSize.width, height: Self.petSize.height)
    ...
    @ViewBuilder
    private func petImage(for visualState: DesktopPetVisualState, tint: Color) -> some View {
        if let image = DesktopPetImageStore.image(for: visualState) {
            Image(nsImage: image)
                .resizable()
                .interpolation(.high)
                .antialiased(true)
    ...
    private func busyDots(at date: Date, tint: Color) -> some View {
        let activeIndex = Int(date.timeIntervalSinceReferenceDate * 5) % 3
        return HStack(spacing: 4) {
            ForEach(0..<3, id: \.self) { index in
                Circle()
                    .fill(tint.opacity(index == activeIndex ? 0.95 : 0.35))
    ...
    private var statusText: String {
        if showsCompletionState(at: .now) {
            return model.lang.t("desktopPet.status.success")
        }

        if model.liveAttentionCount > 0 {
            return model.lang.t("desktopPet.status.attention", model.liveAttentionCount)
        }

        if model.liveSessionCount > 0 {
            return model.lang.t("desktopPet.status.busy", model.liveSessionCount)
        }

        return model.lang.t("desktopPet.status.idle")
    }

    private var statusTint: Color {
        statusTint(for: showsCompletionState(at: .now) ? .success : nil)
    }

    private func statusTint(for visualState: DesktopPetVisualState?) -> Color {
        ...
        if let phase = spotlightSession?.phase {
            return model.isCustomAppearance
                ? model.statusColor(for: phase)
                : defaultStatusColor(for: phase)
        }

        return .white.opacity(0.56)
    }

    private func visualState(at date: Date) -> DesktopPetVisualState {
        if showsCompletionState(at: date) {
            return .success
        }
        ...
        let slot = Int(date.timeIntervalSinceReferenceDate / DesktopPetMetrics.idleSwapInterval)
        return slot.isMultiple(of: 2) ? .idleWave : .idlePlay
    }

    private func showsCompletionState(at _: Date) -> Bool {
        model.completionNotificationSession != nil
    }

    private func defaultStatusColor(for phase: SessionPhase) -> Color {
        switch phase {
        case .running:
...
private enum DesktopPetImageStore {
    private static let idleWaveImage = loadImage(named: "desktop-pet-idle-wave")
    private static let idlePlayImage = loadImage(named: "desktop-pet-idle-play")
    private static let legacyIdleImage = loadImage(named: "desktop-pet-idle")
    private static let busyImage = loadImage(named: "desktop-pet-busy")
    private static let successImage = loadImage(named: "desktop-pet-success")

    static func image(for visualState: DesktopPetVisualState) -> NSImage? {
        switch visualState {
        case .idleWave:
            return idleWaveImage ?? legacyIdleImage ?? idlePlayImage
        case .idlePlay:
            return idlePlayImage ?? legacyIdleImage ?? idleWaveImage
        case .busy:
            return busyImage ?? idleWaveImage ?? legacyIdleImage
        case .success:
            return successImage ?? idleWaveImage ?? legacyIdleImage
        }
    }

    private static func loadImage(named name: String) -> NSImage? {
        let resourceName = ["\(name)@4x", "\(name)@2x", name].first {
            Bundle.appResources.url(forResource: $0, withExtension: "png") != nil
        } ?? name
        guard let url = Bundle.appResources.url(forResource: resourceName, withExtension: "png") else {
            return nil
        }
        let image = NSImage(contentsOf: url)
        image?.size = DesktopPetMetrics.petSize
        return image
    }
}
...
struct IslandPanelView: View {
    ...
    private var scoutTint: Color {
        ...
        if sessions.contains(where: { $0.phase == .running }) {
            return Color(red: 0.26, green: 0.91, blue: 0.42) // #42E86B live green
        }
        if !sessions.isEmpty {
    ...
    private var closedNotchHeight: CGFloat {
        guard let screen = targetOverlayScreen ?? NSScreen.screens.first(where: { $0.safeAreaInsets.top > 0 }) else {
            return 24
        }

        if usesOpenedVisualState || hasClosedPresence {
            return screen.islandClosedHeight
        }

        return screen.notchSize.height
    }
    ...
    private static let sessionListRefreshInterval: TimeInterval = 1

    private var sessionList: some View {
        TimelineView(.periodic(from: .now, by: Self.sessionListRefreshInterval)) { context in
    ...
    private func openedHeaderMetrics(for totalWidth: CGFloat) -> OpenedHeaderMetrics {
        ...
        let notchCenterX = screen.notchCenterX
        let panelMinX = notchCenterX - (totalWidth / 2)
        ...
        let notchLeftEdge = notchCenterX - fallbackNotchHalfWidth
        let notchRightEdge = notchCenterX + fallbackNotchHalfWidth
...
private struct IslandSessionRow: View {
    ...
    private func rowBody(referenceDate: Date) -> some View {
        ...
        return VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .top, spacing: 12) {
                statusRail(for: presence, showsLine: showsExpandedContent || isActionable)

                VStack(alignment: .leading, spacing: 6) {
                    HStack(alignment: .firstTextBaseline, spacing: 12) {
                        Text(session.spotlightHeadlineText)
                            .font(.system(size: isActionable ? 14.5 : 13.5, weight: .semibold))
                        ...
                        HStack(spacing: 6) {
                            ...
                            if session.canJumpToTerminal {
                                JumpButton(action: onJump)
                            }
                            if let onDelete {
                                DeleteSessionButton(action: onDelete)
                            }
                        }
                    }
                    ...
                    if showsExpandedContent || isActionable,
                       let promptLine = session.spotlightPromptLineText ?? expandedPromptLineText {
                        Text(promptLine)
                            .font(.system(size: 11, weight: .medium))
                            .foregroundStyle(.white.opacity(presence == .inactive ? 0.38 : 0.56))
                            .lineLimit(1)
                    }
            ...
            .padding(.vertical, isActionable ? 12 : 10)
        ...
        .background(
            Group {
                if isHighlighted || isSelected {
                    RoundedRectangle(cornerRadius: isActionable ? 18 : 12, style: .continuous)
                        .fill(Color.white.opacity(rowHighlightOpacity))
                }
            }
        )
        .compositingGroup()
        .overlay(alignment: .bottom) {
            if !isActionable {
                Rectangle()
                    .fill(Color.white.opacity(isHighlighted ? 0.035 : 0.018))
                    .frame(height: 1)
            }
        }
    ...
    private var approvalActionBody: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    Button {
                        ...
                        IslandShortcutButtonLabel(title: denyButtonTitle, shortcut: "⌘N")
                    }
                    ...
                    Button {
                        ...
                        IslandShortcutButtonLabel(title: allowButtonTitle, shortcut: "⌘Y")
                    }
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .strokeBorder(.orange.opacity(0.14))
            )
        }
    }
    ...
    private var commandPreviewText: String {
        ...
        if let preview, !preview.isEmpty {
            return preview.hasPrefix("$") ? preview : "$ \(preview)"
        }
    ...
    private var permissionSummaryText: String {
        if let path = session.permissionRequest?.affectedPath.trimmedForNotificationCard, !path.isEmpty {
            return "\(commandLabel) \(path)"
        }

        if let title = session.permissionRequest?.title.trimmedForNotificationCard, !title.isEmpty {
            return title
        }
    ...
    private var denyButtonTitle: String {
        let title = session.permissionRequest?.secondaryActionTitle.trimmedForNotificationCard ?? "Deny"
        return title.caseInsensitiveCompare("No") == .orderedSame ? "Deny" : title
    }

    private var allowButtonTitle: String {
        let title = session.permissionRequest?.primaryActionTitle.trimmedForNotificationCard ?? "Allow"
        return title.caseInsensitiveCompare("Yes") == .orderedSame ? "Allow" : title
    }
    ...
    private func statusRail(for presence: IslandSessionPresence, showsLine: Bool) -> some View {
        let tint = statusTint(for: presence)
        return VStack(spacing: 5) {
            IslandPixelGlyph(
                tint: tint,
                style: .steps,
                isAnimating: presence == .running || session.phase.requiresAttention,
                width: 22,
                height: 13
            )
            .opacity(presence == .inactive ? 0.48 : 1)

            RoundedRectangle(cornerRadius: 1, style: .continuous)
                .fill(tint.opacity(showsLine ? (presence == .inactive ? 0.34 : 0.82) : 0))
                .frame(width: 2, height: showsLine ? 26 : 0)
        }
        .frame(width: 24)
        .padding(.top, 2)
    }
    ...
    private var rowHighlightOpacity: Double {
        if isHighlighted {
            return isActionable ? 0.045 : 0.035
        }

        return isActionable ? 0.034 : 0.026
    }
    ...
    private func compactBadge(
        ...
        .padding(.vertical, 3)
        .background(badgeBackgroundColor(for: title, presence: presence), in: Capsule())
    }

    private func headlineColor(for presence: IslandSessionPresence) -> Color {
        presence == .inactive ? .white.opacity(0.56) : .white.opacity(0.92)
    }

    private func badgeTextColor(for presence: IslandSessionPresence) -> Color {
        presence == .inactive ? .white.opacity(0.36) : .white.opacity(0.68)
    }

    private func badgeBackgroundColor(for title: String, presence: IslandSessionPresence) -> Color {
        if title.caseInsensitiveCompare(session.tool.displayName) == .orderedSame {
            return Color(red: 0.08, green: 0.27, blue: 0.66)
                .opacity(presence == .inactive ? 0.4 : 0.94)
        }

        return Color(red: 0.13, green: 0.13, blue: 0.14)
            .opacity(presence == .inactive ? 0.72 : 0.96)
    }
    ...
    private func activityColor(for presence: IslandSessionPresence) -> Color {
        ...
        case .idle:
            .white.opacity(0.46)
        case .ready:
            presence == .inactive ? .orange.opacity(0.72) : statusTint(for: presence)
        }
    }
}
...
private struct IslandShortcutButtonLabel: View {
    let title: String
    let shortcut: String
...
// MARK: - Agent Deck icon (left side of closed notch)

private struct AgentDeckIcon: View {
    let size: CGFloat
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: "trash.fill")
                .font(.system(size: 10.5, weight: .semibold))
                .foregroundStyle(Color.red.opacity(isHovered ? 0.82 : 0.54))
                .frame(width: 20, height: 18)
                .background(
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        .fill(Color.red.opacity(isHovered ? 0.13 : 0.07))
                )
                .overlay(
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        .strokeBorder(Color.red.opacity(isHovered ? 0.16 : 0.08))
                )
        }
        .buttonStyle(.plain)
        .onHover { isHovered = $0 }
    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: "terminal.fill")
                ...
                .foregroundStyle(.white.opacity(isHovered ? 0.88 : 0.64))
                .frame(width: 20, height: 18)
                .background(
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                ...
                .overlay(
                    RoundedRectangle(cornerRadius: 5, style: .continuous)
                        .strokeBorder(Color.white.opacity(isHovered ? 0.14 : 0.08))
                )
        }
        .buttonStyle(.plain)
        .onHover { isHovered = $0 }
