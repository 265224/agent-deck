...
enum OverlayDisplayResolver {
    ...
    static func diagnostics(
        ...
        panelSize: NSSize,
        presentationMode: OverlayPresentationMode = .notch,
        overlayFrameOverride: NSRect? = nil
        ...
        let overlayFrame = overlayFrameOverride
            ?? frame(for: screen, panelSize: panelSize, presentationMode: presentationMode)
        ...
        return OverlayPlacementDiagnostics(
            ...
            mode: placementMode(for: screen, presentationMode: presentationMode),
    ...
    static func frame(
        ...
        let width = min(panelSize.width, max(120, screen.visibleFrame.width - 64))
        let height = panelSize.height

        if presentationMode == .desktopPet {
            let x = max(screen.visibleFrame.minX + 16, screen.visibleFrame.maxX - width - 28)
            let y = max(
                screen.visibleFrame.minY + 28,
                min(screen.visibleFrame.minY + 52, screen.visibleFrame.maxY - height - 28)
            )
            return NSRect(x: x, y: y, width: width, height: height)
        }

        let x = screen.notchCenterX - (width / 2)

        let y: CGFloat
        switch placementMode(for: screen, presentationMode: presentationMode) {
        case .notch:
            y = screen.frame.maxY - height
        case .topBar:
            y = screen.visibleFrame.maxY - height - 18
        case .desktopPet:
            y = screen.visibleFrame.minY + 52
        }
    ...
    static func placementMode(
        ...
        return isNotched(screen) ? .notch : .topBar
    }
