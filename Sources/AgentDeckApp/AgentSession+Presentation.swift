...
extension AgentSession {
    ...
    var spotlightTerminalLabel: String? {
        guard let jumpTarget,
              let terminalApp = displayableTerminalAppName(from: jumpTarget.terminalApp) else {
            return nil
        }

        return "\(terminalApp) · \(jumpTarget.workspaceName)"
    }
    ...
    var spotlightTerminalBadge: String? {
        ...
        guard let badge = displayableTerminalAppName(from: terminalApp) else {
            return nil
        }

        let normalizedBadge = normalizedBadgeIdentity(badge)
        let normalizedTool = normalizedBadgeIdentity(tool.displayName)
        if normalizedBadge == normalizedTool {
            return nil
        }

        if tool == .catPaw, normalizedBadge.contains("catpaw") {
            return nil
        }
    ...
    var canJumpToTerminal: Bool {
        ...
        return displayableTerminalAppName(from: terminalApp) != nil
    }
    ...
    var spotlightHeadlineText: String {
        if let threadName = codexThreadName {
            return threadName
        }
    ...
    var codexThreadName: String? {
        let threadName = codexMetadata?.threadName?.trimmedForSurface
        guard let threadName, !threadName.isEmpty else {
            return nil
        }
    ...
    private func displayableTerminalAppName(from terminalApp: String) -> String? {
        let trimmed = terminalApp.trimmedForSurface
    ...
    private func normalizedBadgeIdentity(_ value: String) -> String {
        value
            .trimmedForSurface
            .lowercased()
