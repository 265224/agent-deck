...
final class AppModel {
    ...
    private static let overlayPresentationModeDefaultsKey = "overlay.presentation.mode"
    ...
    private static let hasShownInitialSettingsDefaultsKey = "hasShownInitialSettings"
    private static let legacyHasCompletedOnboardingDefaultsKey = "hasCompletedOnboarding"
    private static let legacyOnboardingVersionDefaultsKey = "onboardingVersion"
    ...
    var hasShownInitialSettings: Bool = false {
        didSet {
            guard hasFinishedInit, hasShownInitialSettings != oldValue else { return }
            UserDefaults.standard.set(hasShownInitialSettings, forKey: Self.hasShownInitialSettingsDefaultsKey)
        }
    }
    ...
    var overlayPresentationMode: OverlayPresentationMode = .notch {
        didSet {
            guard overlayPresentationMode != oldValue else { return }
            guard hasFinishedInit else { return }
            UserDefaults.standard.set(overlayPresentationMode.rawValue, forKey: Self.overlayPresentationModeDefaultsKey)
            overlay.handlePresentationModeChanged()
        }
    }
    ...
    @ObservationIgnored
    var openControlCenterWindow: (() -> Void)?
    ...
    init(
        ...
        if UserDefaults.standard.object(forKey: Self.hasShownInitialSettingsDefaultsKey) != nil {
            hasShownInitialSettings = UserDefaults.standard.bool(forKey: Self.hasShownInitialSettingsDefaultsKey)
        } else {
            hasShownInitialSettings =
                UserDefaults.standard.bool(forKey: Self.legacyHasCompletedOnboardingDefaultsKey)
                || UserDefaults.standard.integer(forKey: Self.legacyOnboardingVersionDefaultsKey) > 0
        }
        ...
        overlayPresentationMode = OverlayPresentationMode(
            rawValue: UserDefaults.standard.string(forKey: Self.overlayPresentationModeDefaultsKey) ?? ""
        ) ?? .notch
    ...
    var completionNotificationSession: AgentSession? {
    ...
    func showSettings() {
        openSettingsWindow?()
        if let window = NSApp.windows.first(where: { $0.title == "Agent Deck Settings" }) {
    ...
    func markInitialSettingsShown() {
        ...
        UserDefaults.standard.set(true, forKey: Self.hasShownInitialSettingsDefaultsKey)
    }

    func showControlCenter() {
        openControlCenterWindow?()
        guard let window = NSApp.windows.first(where: { $0.title == "Agent Deck Debug" }) else {
            NSApp.activate(ignoringOtherApps: true)
            return
        }
    ...
    func approvePermission(for sessionID: String, action: ApprovalAction) {
        ...
        switch action {
        case .deny:
            resolution = .deny(message: "Permission denied in Agent Deck.", interrupt: false)
    ...
    func deleteSession(_ sessionID: String) {
        guard state.deleteSession(id: sessionID) else {
            return
        }
    ...
    private func permissionResolution(for approved: Bool) -> PermissionResolution {
        ...
        return .deny(message: "Permission denied in Agent Deck.", interrupt: false)
    }
    ...
    private func applyStartupDiscoveryPayload(_ payload: SessionDiscoveryCoordinator.StartupDiscoveryPayload) {
        ...
        if payload.hooksBinaryURL != nil {
            Task { @MainActor [weak self] in
                ...
                if !self.catPawHooksInstalled { self.installCatPawHooks() }
    ...
    private func computeSessionBuckets() -> (primary: [AgentSession], overflow: [AgentSession]) {
        let now = Date.now
        let displayableSessions = state.sessions.filter(shouldDisplayInSessionBuckets)
        let rankedSessions = displayableSessions.sorted { lhs, rhs in
        ...
        for session in rankedSessions where isPrimaryIslandSession(session) {
    ...
    private func shouldDisplayInSessionBuckets(_ session: AgentSession) -> Bool {
        !isEmptyCodexBootstrapSession(session)
    }

    private func isEmptyCodexBootstrapSession(_ session: AgentSession) -> Bool {
        guard session.tool == .codex,
              !session.isDemoSession,
              session.phase == .running,
              session.permissionRequest == nil,
              session.questionPrompt == nil else {
            return false
        }

        let metadata = session.codexMetadata
        let hasVisibleMetadata = [
            metadata?.initialUserPrompt,
            metadata?.lastUserPrompt,
            metadata?.lastAssistantMessage,
            metadata?.currentTool,
            metadata?.currentCommandPreview,
        ]
        .contains { value in
            value?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty == false
        }
        guard !hasVisibleMetadata else {
            return false
        }

        let summary = session.summary.trimmingCharacters(in: .whitespacesAndNewlines)
        return summary.isEmpty
            || summary.hasPrefix("Started Codex session in ")
            || summary.hasPrefix("Resumed Codex session in ")
    }

    private func isPrimaryIslandSession(_ session: AgentSession) -> Bool {
