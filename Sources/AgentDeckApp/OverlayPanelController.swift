...
final class OverlayPanelController {
    ...
    private static let notificationCardMinContentHeight: CGFloat = 210
    ...
    private static let desktopPetOriginDefaultsKey = "overlay.desktopPet.origin"
    ...
    func ensurePanel(model: AppModel, preferredScreenID: String?) {
        ...
        configurePanelInteractivity(panel, interactive: model.notchStatus == .opened)
    ...
    func hide() {
        ...
        configurePanelInteractivity(panel, interactive: false)
    }
    ...
    func setInteractive(_ interactive: Bool) {
        ...
        configurePanelInteractivity(panel, interactive: interactive)
    ...
    func placementDiagnostics(preferredScreenID: String?) -> OverlayPlacementDiagnostics? {
        ...
        return OverlayDisplayResolver.diagnostics(
            ...
            panelSize: panelSize,
            presentationMode: model?.overlayPresentationMode ?? .notch
        )
    }
    ...
    private func makePanel(model: AppModel) -> NotchPanel {
        ...
        let hostingView = NotchHostingView(rootView: OverlayRootView(model: model))
    ...
    private func positionPanel(
        ...
        return OverlayDisplayResolver.diagnostics(
            ...
            presentationMode: model?.overlayPresentationMode ?? .notch,
            overlayFrameOverride: panel.frame
        )
    }
    ...
    private func configurePanelInteractivity(_ panel: NSPanel, interactive: Bool) {
        let shouldReceiveMouse = interactive || model?.overlayPresentationMode == .desktopPet
        panel.ignoresMouseEvents = !shouldReceiveMouse
        panel.acceptsMouseMovedEvents = shouldReceiveMouse
    }
    ...
    private func computeNotchRect(screen: NSScreen?) {
        ...
        let notchX = screen.notchCenterX - notchSize.width / 2
    ...
    private func handleMouseMoved(_ screenLocation: NSPoint) {
        ...
        if model.overlayPresentationMode == .notch,
    ...
    private func handleMouseDown(_ screenLocation: NSPoint) {
        ...
        if model.overlayPresentationMode == .desktopPet,
           model.notchStatus == .closed {
            return
        }
        ...
        if model.overlayPresentationMode == .desktopPet,
           let panel {
            return Self.rectContainsIncludingEdges(
                desktopPetClosedSurfaceRect(in: panel.frame),
                point: screenPoint
            )
        }
    ...
    func contentRect(for model: AppModel, in bounds: NSRect) -> NSRect? {
        if model.overlayPresentationMode == .desktopPet {
            ...
            case .opened:
                return desktopPetExpandedSurfaceRect(in: bounds)
            case .closed, .popping:
                return desktopPetClosedSurfaceRect(in: bounds)
            }
        }
    ...
    private func panelFrame(for model: AppModel?, on screen: NSScreen) -> NSRect {
        if model?.overlayPresentationMode == .desktopPet {
            return desktopPetPanelFrame(for: model, on: screen, currentFrame: panel?.frame)
        }

        let size = panelSize(for: model, on: screen)
        return NSRect(
            x: screen.notchCenterX - size.width / 2,
    ...
    private func panelSize(for model: AppModel?, on screen: NSScreen) -> CGSize {
        if model?.overlayPresentationMode == .desktopPet {
            return desktopPetPanelSize(for: model, on: screen)
        }
    ...
    private func openedContentHeight(for model: AppModel) -> CGFloat {
        ...
        if isNotificationMode {
            ...
            if model.measuredNotificationContentHeight > 0 {
                return max(
                    model.measuredNotificationContentHeight + 28,
                    Self.notificationCardMinContentHeight
                )
            }
            ...
            if let actionableID,
               let session = model.state.session(id: actionableID) {
                ...
                return max(
                    rowHeight + bodyHeight + Self.openedContentVerticalInsets,
                    Self.notificationCardMinContentHeight
                )
            }
            return 300
    ...
    private func desktopPetClosedSurfaceRect(in bounds: NSRect) -> NSRect {
        bounds
    }

    private func desktopPetExpandedSurfaceRect(in bounds: NSRect) -> NSRect {
        bounds.insetBy(dx: DesktopPetMetrics.expandedPanelInset, dy: 8)
    }

    private func desktopPetPanelSize(for model: AppModel?, on screen: NSScreen) -> CGSize {
        guard model?.notchStatus == .opened else {
            return DesktopPetMetrics.petSize
        }

        return CGSize(
            width: min(
                DesktopPetMetrics.expandedPanelSize.width,
                max(DesktopPetMetrics.petSize.width, screen.visibleFrame.width - 32)
            ),
            height: min(
                DesktopPetMetrics.expandedPanelSize.height,
                max(DesktopPetMetrics.petSize.height, screen.visibleFrame.height - 48)
            )
        )
    }

    private func desktopPetPanelFrame(
        for model: AppModel?,
        on screen: NSScreen,
        currentFrame: NSRect?
    ) -> NSRect {
        let size = desktopPetPanelSize(for: model, on: screen)
        let petOrigin = desktopPetOrigin(on: screen, currentFrame: currentFrame)

        if model?.notchStatus == .opened {
            let frame = NSRect(
                x: petOrigin.x + (DesktopPetMetrics.petSize.width / 2) - (size.width / 2),
                y: petOrigin.y - DesktopPetMetrics.expandedBottomPadding,
                width: size.width,
                height: size.height
            )
            return clamp(rect: frame, to: screen.visibleFrame.insetBy(dx: 8, dy: 8))
        }

        let origin = clampPetOrigin(petOrigin, on: screen)
        return NSRect(origin: origin, size: size)
    }

    private func desktopPetOrigin(on screen: NSScreen, currentFrame: NSRect?) -> NSPoint {
        if let currentFrame,
           model?.overlayPresentationMode == .desktopPet,
           currentFrame.width > 0,
           currentFrame.height > 0 {
            if isDesktopPetClosedFrame(currentFrame) {
                return clampPetOrigin(currentFrame.origin, on: screen)
            }

            return clampPetOrigin(
                NSPoint(
                    x: currentFrame.midX - (DesktopPetMetrics.petSize.width / 2),
                    y: currentFrame.minY + DesktopPetMetrics.expandedBottomPadding
                ),
                on: screen
            )
        }

        if let saved = Self.loadDesktopPetOrigin() {
            return clampPetOrigin(saved, on: screen)
        }

        return NSPoint(
            x: screen.visibleFrame.maxX - DesktopPetMetrics.petSize.width - DesktopPetMetrics.defaultTrailingPadding,
            y: screen.visibleFrame.minY + DesktopPetMetrics.defaultBottomPadding
        )
    }

    private func isDesktopPetClosedFrame(_ frame: NSRect) -> Bool {
        abs(frame.width - DesktopPetMetrics.petSize.width) < 1
            && abs(frame.height - DesktopPetMetrics.petSize.height) < 1
    }

    private func clampPetOrigin(_ origin: NSPoint, on screen: NSScreen) -> NSPoint {
        let visibleFrame = screen.visibleFrame.insetBy(dx: 8, dy: 8)
        return NSPoint(
            x: min(max(origin.x, visibleFrame.minX), visibleFrame.maxX - DesktopPetMetrics.petSize.width),
            y: min(max(origin.y, visibleFrame.minY), visibleFrame.maxY - DesktopPetMetrics.petSize.height)
        )
    }
    ...
    func persistDesktopPetPosition() {
        guard let panel,
              model?.overlayPresentationMode == .desktopPet,
              let screen = panel.screen ?? resolveTargetScreen() else {
            return
        }

        let origin = desktopPetOrigin(on: screen, currentFrame: panel.frame)
        let clamped = clampPetOrigin(origin, on: screen)
        UserDefaults.standard.set(NSStringFromPoint(clamped), forKey: Self.desktopPetOriginDefaultsKey)
    }

    private static func loadDesktopPetOrigin() -> NSPoint? {
        guard let rawValue = UserDefaults.standard.string(forKey: desktopPetOriginDefaultsKey) else {
            return nil
        }
        return NSPointFromString(rawValue)
    }
...
final class NotchHostingView<Content: View>: NSHostingView<Content> {
    ...
    override func mouseDragged(with event: NSEvent) {
        guard let controller = notchController,
              controller.model?.overlayPresentationMode == .desktopPet,
              let window else {
        ...
        controller.persistDesktopPetPosition()
    }
...
extension NSScreen {
    var notchSize: CGSize {
        ...
        let notchWidth = Self.computeNotchWidth(
            screenFrame: frame,
    ...
    ///
    /// On notch screens, cover the full top reserved area. AppKit can report
    /// `visibleFrame.maxY` 1pt below `safeAreaInsets.top`; using the larger value
    /// avoids a thin visible seam between the island bottom and the menu-bar edge.
    ...
    static func computeIslandClosedHeight(
        ...
        if safeAreaInsetsTop > 0 {
            return max(safeAreaInsetsTop, topStatusBarHeight)
        }
        return topStatusBarHeight
    }

    /// Pure helper so notch width math can be verified without real screen hardware.
    static func computeNotchWidth(
        screenFrame: NSRect,
        ...
        return max(0, screenFrame.width - leftPadding - rightPadding)
    }

    /// The physical notch is not always exactly at `screen.frame.midX` because
    /// AppKit can report left/right auxiliary menu-bar areas that differ by 1pt.
    static func computeNotchCenterX(
        screenFrame: NSRect,
        auxiliaryTopLeftArea: NSRect?,
        auxiliaryTopRightArea: NSRect?
    ) -> CGFloat {
