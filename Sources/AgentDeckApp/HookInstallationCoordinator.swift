...
final class HookInstallationCoordinator {
    ...
    var claudeHookStatusSummary: String {
        ...
        if hooksBinaryURL == nil {
            return "Build Agent Deck hooks before installing."
        }
    ...
    var claudeUsageStatusSummary: String {
        ...
        if status.managedStatusLineNeedsRepair {
            return "Agent Deck detected a missing managed Claude status line script and will repair it automatically."
        }

        if status.hasConflictingStatusLine {
            return "Agent Deck will not overwrite an existing Claude status line automatically."
        }
    ...
    var catPawHookStatusTitle: String {
        guard let status = catPawHookStatus else { return "CatPaw hooks loading" }
        return status.managedHooksPresent ? "CatPaw hooks installed" : "CatPaw hooks not installed"
    }

    var catPawHookStatusSummary: String {
        guard let status = catPawHookStatus else {
            return "Reading CatPaw global hooks.json."
        }
        ...
        return status.managedHooksPresent ? "managed hooks present" : "no managed CatPaw hooks"
    }
    ...
    func installCodexHooks() {
        guard let hooksBinaryURL else {
            onStatusMessage?("Could not find a local Agent Deck hooks binary. Build the package first.")
            return
        }
    ...
    func installCatPawHooks() {
        ...
        updateCatPawHooks(userMessage: "Installing CatPaw hooks.") { manager in
            try manager.install(hooksBinaryURL: hooksBinaryURL)
        }
    }

    func uninstallCatPawHooks() {
        updateCatPawHooks(userMessage: "Removing CatPaw hooks.") { manager in
            try manager.uninstall()
        }
    }
    ...
    private func loadBundledOpenCodePlugin() -> Data? {
        ...
        if let url = Bundle.appResources.url(forResource: "agent-deck-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }

        // Fallback: Bundle.main for Xcode builds
        if let url = Bundle.main.url(forResource: "agent-deck-opencode", withExtension: "js") {
            return try? Data(contentsOf: url)
        }
