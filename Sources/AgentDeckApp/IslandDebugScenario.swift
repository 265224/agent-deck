...
enum IslandDebugScenario: String, CaseIterable, Identifiable {
    ...
    var summary: String {
        switch self {
        case .emptyClosed:
            "Collapsed physical notch with no live sessions."
        case .closed:
            "Collapsed idle/running notch with live count and attention affordance."
        case .sessionList:
            "Manual expanded list with active running session rows."
        case .approvalCard:
    ...
    static func runningSession(now: Date) -> AgentSession {
        AgentSession(
            ...
            codexMetadata: CodexSessionMetadata(
                ...
                currentCommandPreview: "sed -n '1,260p' Sources/AgentDeckApp/Views/ControlCenterView.swift"
            )
        )
    }

    static func backgroundRunningSession(
        id: String,
        workspace: String,
    ...
    static func approvalSession(now: Date) -> AgentSession {
        AgentSession(
            ...
            permissionRequest: PermissionRequest(
                ...
                summary: "Allow exec_command to rewrite ControlCenterView.swift?",
                affectedPath: "Sources/AgentDeckApp/Views/ControlCenterView.swift",
...
[README.md](/Users/wangruobing/Personal/agent-deck/README.md) 的现有改动已经单独提交了，commit 是 `f196316`，message 是 `docs: update readme tagline`。
