...
enum HarnessArtifactRecorder {
    static func record(
        ...
        let report = HarnessArtifactReport(
            ...
            showedSettings: configuration.shouldShowSettings,
    ...
    private static func recognizedWindowKind(for window: NSWindow) -> String? {
        ...
        if window.title == "Agent Deck Debug" {
            return "control-center"
        }

        if window.title == "Agent Deck Settings" {
            return "settings"
        }
        ...
        if window.frame.width >= 820, window.frame.height >= 560 {
            return "settings"
        }
