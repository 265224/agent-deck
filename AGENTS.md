...
- Supported agents: `Claude Code`, `Codex`, `OpenCode`, `Cursor`, `CatPaw`, `Qoder`, `Qwen Code`, `Factory`, `CodeBuddy`.
...
- Treat `~/Applications/Agent Deck.app` as a local development bundle wrapper around the repo-built `AgentDeckApp`, not as a separate product line.
- Use `Agent Deck.app` for manual OSS app verification when bundle semantics, LaunchServices, or installed-hook behavior matter.
- When the user asks to launch or restart `Agent Deck.app`, refresh the bundle from the current repo first with `zsh scripts/launch-dev-app.sh` instead of only running `open -na`. Opening the bundle alone can relaunch a stale binary.
...
- Treat `/Applications/Agent Deck.app` and `https://agentdeck.app/` as closed-source reference baselines only. They are behavior benchmarks, not the development runtime for this repository.
...
- Official product reference: `https://agentdeck.app/`
