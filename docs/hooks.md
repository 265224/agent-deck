...
AgentDeck receives hook events from AI agents (Codex / Claude Code / Gemini CLI / CatPaw) via the `AgentDeckHooks` CLI. The CLI forwards payloads to the app over a Unix socket and, when necessary, writes a directive back to stdout so the agent can act on it (e.g. block a tool call).

## Architecture

```
Agent (Codex / Claude Code / Gemini CLI / CatPaw)
  │  stdin: JSON payload
  ▼
AgentDeckHooks CLI  (--source codex | --source claude | --source gemini | --source catpaw)
  │  Unix socket
  ▼
...
## CatPaw Hooks (`--source catpaw`)

**Payload type**: `CatPawHookPayload`
**Source**: [`Sources/AgentDeckCore/CatPawHooks.swift`](../Sources/AgentDeckCore/CatPawHooks.swift)

### Events

| `hook_event_name` | When it fires | Current AgentDeck behavior |
|---|---|---|
| `beforeSubmitPrompt` | User submits a prompt | Creates or restores the CatPaw session, updates prompt metadata, keeps the turn running |
| `beforeShellExecution` | Before a shell command executes | Updates the running command preview and allows the command |
| `afterShellExecution` | After a shell command completes | Updates the activity summary from command output |
| `beforeMCPExecution` | Before an MCP tool call | Updates current tool metadata and allows the call |
| `afterMCPExecution` | After an MCP tool call | Updates the activity summary from the result |
| `beforeReadFile` | Before a file read | Updates current file-read activity and allows the read |
| `afterFileEdit` | After a file edit | Updates edit activity metadata |
| `afterAgentThought` | CatPaw emits thought text | Updates activity text without blocking CatPaw |
| `afterAgentResponse` | CatPaw emits response text | Marks the turn completed and emits a completion card |
| `stop` | CatPaw completes, aborts, or errors a turn | Marks the turn completed and returns an empty follow-up directive |
...
|---|---|---|
| `hook_event_name` | `hookEventName` | Event type |
| `conversation_id` | `conversationId` | Session identifier |
| `suggest_uuid` | `suggestUuid` | CatPaw suggestion / generation identifier |
| `model` | `model` | Model name |
| `catpaw_version` | `catpawVersion` | CatPaw app / extension version |
| `workspace_roots` | `workspaceRoots` | Workspace roots for title and jump-back |
| `mis` | `mis` | Meituan user identifier when provided by CatPaw |
| `agent_name` | `agentName` | CatPaw agent label |
| `is_sub_agent` | `isSubAgent` | Whether the event came from a sub-agent |
| `command` | `command` | Shell command text |
| `cwd` | `cwd` | Working directory |
| `tool_name` | `toolName` | MCP tool name |
| `tool_input` | `toolInput` | MCP tool input preview |
| `file_path` | `filePath` | File path for file-read/edit events |
| `prompt` | `prompt` | User prompt text |
| `text` | `text` | Response or thought text |
| `status` | `status` | Stop status, such as completed, aborted, or error |

### Current feature coverage

- Managed hook installation into CatPaw's global `hooks.json`
- Session list, activity, completion cards, and workspace jump-back from CatPaw hook events
- Fail-open stdout behavior for CatPaw before hooks and `stop`
- No CatPaw usage / quota tracking
...
| CatPaw | All events | Bridge default |
...
| File | Responsibility |
|---|---|
| [`Sources/AgentDeckHooks/main.swift`](../Sources/AgentDeckHooks/main.swift) | Hook CLI entry point — routes to Codex, Claude, Gemini, or CatPaw path |
...
| [`Sources/AgentDeckCore/CatPawHooks.swift`](../Sources/AgentDeckCore/CatPawHooks.swift) | CatPaw payload model, output encoder, metadata helpers |
