...
**10 agents**: Claude Code, Codex, Cursor, CatPaw, Gemini CLI, OpenCode, Qoder, Qwen Code, Factory, CodeBuddy

**16+ terminals & IDEs**: Terminal.app, Ghostty, iTerm2, WezTerm, Zellij, tmux, cmux, Kaku, VS Code, Cursor, CatPaw, Windsurf, Trae, JetBrains IDEs (IDEA, WebStorm, PyCharm, GoLand, CLion, RubyMine, PhpStorm, Rider, RustRover)
...
| **CatPaw** | Supported | Hook integration via CatPaw global `hooks.json`, session tracking, workspace jump-back, no usage tracking |
...
| **CatPaw** | Workspace | Activate workspace via `catpaw` CLI |
...
1. Collect my environment info:
   ...
   - Get the app version: `defaults read ~/Applications/Agent\ Deck.app/Contents/Info.plist CFBundleShortVersionString 2>/dev/null || echo "unknown"`
...
- **CatPaw** — Hook-based integration via CatPaw's global `hooks.json`. Receives shell, MCP, file, prompt, stop, response, and thought events. Session tracking and workspace jump-back are supported. Usage / quota tracking is intentionally not integrated. Use `--source catpaw` with the hooks binary.
