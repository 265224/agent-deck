...
if [[ "$(uname -s)" != "Darwin" ]]; then
    echo "Agent Deck packaging runs only on macOS." >&2
    exit 1
fi

repo_root="$(cd "$(dirname "$0")/.." && pwd)"
app_name="${AGENT_DECK_APP_NAME:-Agent Deck}"
bundle_identifier="${AGENT_DECK_BUNDLE_ID:-app.agentdeck.local}"
version="${AGENT_DECK_VERSION:-0.1.0}"
build_number="${AGENT_DECK_BUILD_NUMBER:-$(git -C "$repo_root" rev-list --count HEAD 2>/dev/null || echo 1)}"
package_root="${AGENT_DECK_PACKAGE_ROOT:-$repo_root/output/package}"
bundle_dir="${AGENT_DECK_BUNDLE_DIR:-$package_root/$app_name.app}"
zip_path="${AGENT_DECK_ZIP_PATH:-$package_root/$app_name.zip}"
dmg_path="${AGENT_DECK_DMG_PATH:-$package_root/$app_name.dmg}"
signing_identity="${AGENT_DECK_SIGN_IDENTITY:-}"
notary_profile="${AGENT_DECK_NOTARY_PROFILE:-}"

dmg_bg_script="$repo_root/scripts/generate_dmg_background.py"
entitlements_path="$repo_root/config/packaging/AgentDeckApp.entitlements"

cd "$repo_root"

arch_flags=()
if [[ "${AGENT_DECK_UNIVERSAL:-false}" == "true" ]]; then
    arch_flags=(--arch arm64 --arch x86_64)
fi

swift build -c release "${arch_flags[@]}" --product AgentDeckApp
swift build -c release "${arch_flags[@]}" --product AgentDeckHooks
swift build -c release "${arch_flags[@]}" --product AgentDeckSetup

build_bin_dir="$(swift build -c release "${arch_flags[@]}" --show-bin-path)"
app_binary="$build_bin_dir/AgentDeckApp"
hooks_binary="$build_bin_dir/AgentDeckHooks"
setup_binary="$build_bin_dir/AgentDeckSetup"
...
cp "$app_binary" "$bundle_dir/Contents/MacOS/AgentDeckApp"
cp "$hooks_binary" "$bundle_dir/Contents/Helpers/AgentDeckHooks"
cp "$setup_binary" "$bundle_dir/Contents/Helpers/AgentDeckSetup"
cp "$brand_icon" "$bundle_dir/Contents/Resources/AgentDeck.icns"
...
spm_resource_bundle="$build_bin_dir/AgentDeck_AgentDeckApp.bundle"
...
verify_errors=0
for required in \
    "Contents/MacOS/AgentDeckApp" \
    "Contents/Helpers/AgentDeckHooks" \
    "Contents/Helpers/AgentDeckSetup" \
    "Contents/Resources/AgentDeck.icns" \
    "Contents/Resources/AgentDeck_AgentDeckApp.bundle" \
; do
...
smoke_binary="$smoke_app/Contents/MacOS/AgentDeckApp"
...
if [[ -n "$signing_identity" ]]; then
    ...
    codesign --force --sign - "$bundle_dir/Contents/Helpers/AgentDeckHooks" 2>/dev/null || true
    codesign --force --sign - "$bundle_dir/Contents/Helpers/AgentDeckSetup" 2>/dev/null || true
